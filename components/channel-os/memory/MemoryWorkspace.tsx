...
export const MemoryWorkspace = () => {
  const { user } = useAuth();
  const [memories, setMemories] = useState<HermesMemory[] | null>(null);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => setMemories((data as HermesMemory[]) ?? []));
  }, [user]);
        ...
        {memories === null ? (
          ...
          <div className="text-center py-20 max-w-md mx-auto">
            ...
            <p className="text-sm text-fg-secondary mt-2">
              Conforme você conversa com o Hermes, ele cria pins de preferências, metas e fatos importantes
              aqui. Você pode editar, deletar e priorizar tudo.
            </p>
          </div>
        ) : (
          memories.map((m) => (
            <Card key={m.id} className="flex items-start gap-3 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Badge variant="brand" size="sm">{KIND_LABEL[m.kind]}</Badge>
